...
class DatabaseManager(object):
    ...
    def Search(self, args, limit=10, offset=0):
        ...
        c.execute(
            'SELECT * FROM songs WHERE online = 1 AND (album LIKE ? OR artist LIKE ? OR title LIKE ? OR path LIKE ?) ORDER BY album, disc, track LIMIT ? OFFSET ?', a)
        results = c.fetchall()
        return self.toDicts(results)
    ...
    def QuickSearch(self, q, limit=10):
        ...
        a = (s, s, s, limit)
        c.execute(
            'SELECT title, album, artist FROM songs WHERE online = 1 AND (album LIKE ? OR artist LIKE ? OR title LIKE ?) GROUP BY album LIMIT ?', a)
        results = c.fetchall()
        return self.toDicts(results)
    ...
    def History(self, voter=None, player="default", limit=10):
        ...
        if voter:
            a = (voter, limit)
            c.execute(
                "SELECT time FROM history WHERE voter = ? GROUP BY time ORDER BY time DESC LIMIT ?", a)
            results = c.fetchall()
            a = (results[-1]["time"], player, voter, limit)
            c.execute(
                "SELECT history.who, history.time, songs.* FROM history INNER JOIN songs ON history.song_id = songs.song_id WHERE history.time >= ? AND history.player_id = ? AND WHERE history.who = ? ORDER BY history.time DESC LIMIT ?", a)
            results = c.fetchall()
            return self.toDicts(results)
        else:
            a = (limit,)
            c.execute(
                "SELECT time FROM history GROUP BY time ORDER BY time DESC LIMIT ?", a)
            results = c.fetchall()
            a = (results[-1]["time"], player, limit)
            c.execute(
                "SELECT history.who, history.time, songs.* FROM history INNER JOIN songs ON history.song_id = songs.song_id WHERE history.time >= ? AND history.player_id = ? ORDER BY history.time DESC LIMIT ?", a)
    ...
    def SongsByVotes(self, player=None):
        c = self.conn.cursor()
        c.execute(
            "SELECT votes.who, votes.priority, votes.time, songs.* FROM votes INNER JOIN songs ON votes.song_id = songs.song_id WHERE votes.player_id = ? ORDER BY votes.priority", [player])
        return self.toDicts(c.fetchall())

    def TopVoted(self, limit=20):
        c = self.conn.cursor()
        c.execute(
            'SELECT title, history.song_id, COUNT(history.song_id) FROM history, songs WHERE songs.song_id = history.song_id GROUP BY history.song_id ORDER BY COUNT(history.song_id) DESC LIMIT ?', [limit])
        return self.toDicts(c.fetchall())
    ...
    def AlbumSearch(self, keyword=""):
        ...
        s = "%%%s%%" % (keyword)
        c.execute(
            'SELECT album, song_id FROM songs WHERE (album LIKE ? OR artist LIKE ?) AND online = 1 GROUP BY album', [s, s])
        return self.toDicts(c.fetchall())
    ...
    def Select(self, field, value, mode="select"):
        ...
        if field == "any":
            where = "OR"
            args = {"artist": value, "album":
                    value, "title": value, "path": value}
        else:
            where = "AND"
    ...
    def NextVote(self, user, player):
        c = self.conn.cursor()
        c.execute(
            "SELECT max(priority) FROM votes WHERE who = ? AND player_id = ?", [user, player])
        v = c.fetchall()
    ...
    def NumVotes(self, user, player):
        c = self.conn.cursor()
        c.execute(
            "SELECT count(*) FROM votes WHERE who = ? AND player_id = ?", [user, player])
        v = c.fetchall()
        if len(v):
    ...
    def AddVote(self, user, player, song, priority):
        c = self.conn.cursor()
        c.execute(
            "INSERT IGNORE INTO votes (song_id, time, player_id, who, priority) VALUES (?, now(), ?, ?, ?)",
            [song, player, user, priority])
        self.conn.commit()

    def Unvote(self, user, song):
        c = self.conn.cursor()
        c.execute(
            "DELETE FROM votes WHERE song_id = ? AND who = ?", [song, user])
        self.conn.commit()

    def DeleteVotes(self, song, player):
        c = self.conn.cursor()
        c.execute(
            "DELETE FROM votes WHERE player_id = ? AND song_id = ?", [player, song])
        self.conn.commit()

    def SetPlayed(self, song, player, who, time):
        c = self.conn.cursor()
        c.execute(
            "INSERT INTO history (song_id, player_id, who, time) VALUES (?, ?, ?, ?)", [song, player, who, time])
        self.conn.commit()
    ...
    def TopArtists(self, who=None, limit=10):
        ...
        if who:
            c.execute(
                "SELECT artist, count(songs.artist) AS count FROM songs, history WHERE songs.song_id = history.song_id AND history.who = ? GROUP BY artist ORDER BY count(songs.artist) DESC LIMIT ?", [who, limit])
        else:
            c.execute(
                "SELECT artist, count(songs.artist) AS count FROM songs, history WHERE songs.song_id = history.song_id GROUP BY artist ORDER BY count(songs.artist) DESC LIMIT ?", [limit])
        return self.toDicts(c.fetchall())
    ...
    def PlaylistContents(self, playlist_id):
        c = self.conn.cursor()
        c.execute(
            "SELECT songs.* FROM playlist_contents INNER JOIN songs ON playlist_contents.song_id = songs.song_id WHERE online = 1 AND playlist_id = ? ORDER BY priority", [playlist_id])
        return self.toDicts(c.fetchall())
    ...
    def NextPlaylistSong(self, playlist_id):
        c = self.conn.cursor()
        c.execute(
            "SELECT max(priority) FROM playlist_contents WHERE playlist_id = ?", [playlist_id])
        v = c.fetchall()
        if len(v) and not v[0]["max(priority)"] is None:
            return v[0]["max(priority)"] + 1
        else:
    ...
    def AddToPlaylist(self, playlist_id, song_id):
        ...
        c = self.conn.cursor()
        c.execute(
            "INSERT INTO playlist_contents (playlist_id, song_id, priority) VALUES (?, ?, ?)",
            [playlist_id, song_id, priority])
        self.conn.commit()

    def RemoveFromPlaylist(self, playlist_id, song_id):
        c = self.conn.cursor()
        c.execute(
            "DELETE FROM playlist_contents WHERE playlist_id = ? AND song_id = ?", [playlist_id, song_id])
        self.conn.commit()

    def CreatePlaylist(self, user, title):
        c = self.conn.cursor()
        c.execute(
            "INSERT INTO playlists (who, title) VALUES (?, ?)", [user, title])
        self.conn.commit()
    ...
    def DeletePlaylist(self, playlist_id):
        ...
        c.execute(
            "DELETE FROM playlist_contents WHERE playlist_id = ?", [playlist_id])
        self.conn.commit()

    def Purge(self, user, player_id):
        c = self.conn.cursor()
        c.execute(
            "DELETE FROM votes WHERE player_id = ? AND who = ?", [player_id, user])
        self.conn.commit()
    ...
    def CreatePlayer(self, player_id, local_id, volume):
        c = self.conn.cursor()
        c.execute(
            "INSERT INTO players (player_id, local_id, volume) VALUES (?, ?, ?)",
            [player_id, local_id, volume])
        self.conn.commit()
    ...
    def UpdateSong(self, song_id, args):
        c = self.conn.cursor()
        valid = ["path", "artist", "albumartist",
                 "album", "title", "disc", "length", "track"]
        a = []
        s = []
        for k, v in args.items():
            if k in valid:
    ...
    def PlayerQueue(self, player):
        ...
        for i in raw:
            if x and x[0]['song_id'] == i['song_id']:
                continue
            outlist.append(i)
            ...
            for i in votes:
                if i['song_id'] == nextsong['song_id']:
                    ...
                    if i['who'] not in voters:
                        voters.append(i['who'])
            payout = (float(nextsong['length']) + 1.0) / (
                float(len(remaining_voters)) + 1.0)
        ...
        for i in playlist:
            x = self.SELECT(
                "votes", {"song_id": i["song_id"], "player_id": player})
            i['who'] = []
            for j in x:
                i['who'].append(j['who'])
    ...
    def UpdateVote(self, song_id, who, player, priority):
        c = self.conn.cursor()
        c.execute(
            "UPDATE votes SET priority = ? WHERE song_id = ? AND who = ? AND player_id = ?",
            [priority, song_id, who, player])
        self.conn.commit()
...
class Sqlite(DatabaseManager):
    ...
    def AddVote(self, user, player, song, priority):
        c = self.conn.cursor()
        try:
            c.execute("INSERT INTO votes (song_id, time, player_id, who, priority) VALUES (?, date('now'), ?, ?, ?)", [
                      song, player, user, priority])
            self.conn.commit()
